...
def top_prefix_three_regime_v2_policy(x: tuple[int, ...]) -> Policy:
    state = canon(x)
    k = len(state)
    if k <= 1:
        return _top_prefix_policy_with_length(state, 0)
    ...
    if all(gap == 0 for gap in gaps):
        length = largest_odd
    elif packet_sizes[0] >= k - 2:
        length = largest_odd
    elif len(packet_sizes) >= 3 and packet_sizes[0] == 1 and packet_sizes[1] >= k - 2:
        length = largest_odd
    elif packet_sizes[:3] == (1, 2, 2):
        length = min(3, largest_odd)
    elif packet_sizes in frontier_mixed_packet_types:
        length = min(3, largest_odd)
    elif gaps[0] >= 2:
        length = 1
    else:
...
def top_prefix_three_regime_v3_policy(x: tuple[int, ...]) -> Policy:
    state = canon(x)
    k = len(state)
    if k <= 1:
        return _top_prefix_policy_with_length(state, 0)
...
def top_prefix_three_regime_v4_policy(x: tuple[int, ...]) -> Policy:
    state = canon(x)
    k = len(state)
    if k <= 1:
        return _top_prefix_policy_with_length(state, 0)
...
def top_prefix_three_regime_v6_policy(x: tuple[int, ...]) -> Policy:
    state = canon(x)
    k = len(state)
    if k <= 1:
        return _top_prefix_policy_with_length(state, 0)
    ...
    if all(gap == 0 for gap in gaps):
        ...
        length = 1
    elif len(packet_sizes) == 2 and min(packet_sizes) >= 3:
        length = largest_odd
    elif len(packet_sizes) >= 3 and packet_sizes[0] >= 2 and packet_sizes[1] >= 2 and packet_sizes[2] >= 2:
        length = broad_length
    elif any(size >= (k + 1) // 2 for size in packet_sizes):
        length = broad_length
    elif len(packet_sizes) >= 2 and packet_sizes[0] + packet_sizes[1] >= k - 1:
...
def top_prefix_three_regime_v7_policy(x: tuple[int, ...]) -> Policy:
    state = canon(x)
    k = len(state)
    if k <= 1:
        return _top_prefix_policy_with_length(state, 0)
    if k <= 8:
        return top_prefix_three_regime_v6_policy(state)

    largest = k - 1
    broad_length = min(5, largest)
    ...
    if all(gap == 0 for gap in gaps):
        ...
        length = min(3, largest)
    elif len(packet_sizes) >= 3 and packet_sizes[0] == 1 and packet_sizes[1] == 2 and packet_sizes[2] >= 3:
        length = min(3, largest)
    elif len(packet_sizes) >= 3 and packet_sizes[0] == 1 and packet_sizes[1] >= 4 and 2 <= packet_sizes[2] <= 3:
        length = min(3, largest)
    elif len(packet_sizes) == 2 and min(packet_sizes) >= 4:
        length = near_global_length
    elif len(packet_sizes) == 3 and min(packet_sizes) >= 3:
        length = near_global_length
    elif len(packet_sizes) >= 3 and packet_sizes[1] >= 4 and packet_sizes[2] >= 3:
        length = near_global_length
    elif packet_sizes == (1, k - 1) and gaps[0] >= 2:
        length = 1
    elif len(packet_sizes) == 2 and min(packet_sizes) >= 3:
